...
const SessionNameModal: React.FC<SessionNameModalProps> = ({ isOpen, onClose, onConfirm, currentName }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[100] p-4">
            <div className="bg-[#1e1e1e] rounded-xl shadow-2xl p-8 w-full max-w-md relative border border-amber-200">
                <button
                    onClick={onClose}
                ...
                <h2 className="text-2xl text-amber-200 font-semibold mb-6 text-center">Sessiyaya Ad Ver</h2>
                ...
                <div className="relative w-full mb-8">
                    <input
                        ...
                        className="
                            w-full bg-transparent border-b-2 border-gray-500 text-white
                            py-2 px-0 focus:outline-none focus:border-amber-400
                            transition-all duration-300 peer
                        "
                    />
                    <div className="
                        absolute bottom-0 left-0 w-0 h-0.5 bg-amber-200 transition-all duration-300
                        peer-focus:w-full peer-focus:left-0
                        peer-not-placeholder-shown:w-full peer-not-placeholder-shown:left-0
                    "></div>
                    <label className="
                        absolute left-0 -top-6 text-gray-400 text-sm transition-all duration-300
